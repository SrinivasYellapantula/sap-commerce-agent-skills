...
description: Prepare and carry out SAP Commerce upgrade implementation work with explicit human review gates. Use when Codex must turn an upgrade analysis into a documented change plan, compare approaches for complex customizations, explain reasoning and risks, wait for final approval before touching code or configuration, then implement and verify only the approved SAP Commerce or composable storefront changes.
---
...
Convert an approved upgrade analysis into an implementation decision pack first, and code changes only after the user explicitly gives final approval to apply them.

## Approval State Machine

1. `analysis received`
   - Read the project baseline and upgrade matrix.
...
3. `approach chosen`
   - Update the pack with the chosen approaches, expected files, migration/data actions, verification plan, rollback notes, and unresolved risks.
...
If final approval has not been given, end with the decision pack and the smallest clear set of decisions the user needs to make next. If final approval has been given, end with the implementation result and verification summary.
