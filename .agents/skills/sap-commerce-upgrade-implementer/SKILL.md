...
- Runtime ASM feedback scan: for every Assisted Service Module issue where ASM does not open, CS-agent login fails, customer emulation starts but OCC calls still run as the real user, or requests miss `emulatedUserId`, scan the backend and storefront ASM surface together. Check enabled `assistedservice*` extensions, ASM OAuth clients and roles, `assistedservicewebservices` CORS/config, custom `AsmEnablerService` launch rules, custom `AsmMainUiComponent` templates and overrides, `CSAgentLoginFormComponent`, `CsAgentAuthService`, `authorizationCodeFlowByDefault`, customer-selection event parameters, ASM session event creation, and every interceptor adding `emulatedUserId`. Prepare the smallest patch that restores the target Spartacus behavior instead of broad refactors.
...
- ASM authorization-code compatibility: when ASM is enabled, inspect custom storefront ASM UI and backend assisted-service configuration together. If `authorizationCodeFlowByDefault` is enabled, custom ASM bars must use the target auth-code CS-agent sign-in path instead of always rendering the legacy password form. Preserve target ASM session event creation, customer-selection deep-link parameters, and `emulatedUserId` request propagation with the smallest patch needed.
