...
3. Filter docs through implementation evidence.
   ...
   - For Assisted Service Module (ASM), inventory both backend and storefront evidence when a storefront root is available. Search for enabled `assistedservice*` extensions, ASM OAuth clients, CS agent roles/groups, CORS entries for `assistedservicewebservices`, custom `AsmEnablerService`, custom ASM main UI components/templates, `CSAgentLoginFormComponent`, `CsAgentAuthService`, `authorizationCodeFlowByDefault`, customer-selection overrides, and interceptors adding `emulatedUserId`. Flag custom ASM UI that still renders the legacy CS-agent password form when authorization-code flow is enabled, custom overrides that omit upgraded ASM session events, and interceptors that add `emulatedUserId` asynchronously after the HTTP request has already continued.
   - For custom SAP security providers, scan deprecated-but-still-compiling SAP security types such as `CoreUserDetails`, especially inside custom `CoreAuthenticationProvider` subclasses. If `CoreUserDetails` is used only to access OTP verification token data, flag migration to `UserVerificationTokenService.lookupTokenWithUser(...)` while keeping custom method contracts on Spring Security `UserDetails`.
...
- When ASM does not open, CS-agent login fails, emulation starts but OCC calls still run as the real user, or requests miss `emulatedUserId`, convert the symptom into a project-wide ASM scan. Check custom ASM module imports, `AsmEnablerService` launch rules, authorization-code sign-in integration, custom `AsmMainUiComponent` templates and overrides, customer-selection event parameters, ASM session event creation, `assistedservicewebservices` CORS/config, and every ASM-related HTTP interceptor.
- When a static analysis, Sonar, deprecation scan, or API compatibility check flags deprecated SAP security types such as `CoreUserDetails`, scan every custom `CoreAuthenticationProvider` subclass for direct casts, constructor calls, method parameters, and OTP token access. Prefer Spring Security `UserDetails` in custom method signatures and use `UserVerificationTokenService.lookupTokenWithUser(...)` for OTP verification token lookup instead of depending on deprecated SAP user-detail internals.
